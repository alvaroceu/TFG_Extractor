...
class TransformerBiLBERTLargeExtractor(ExtractorBase):
    def __init__(self, model_weights_path="trained_models/bilbert_large_qa_weights.pth"):
        print("📥 Cargando Tokenizador y arquitectura BiLBERTLarge...")
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.tokenizer = AutoTokenizer.from_pretrained('deepset/bert-large-uncased-whole-word-masking-squad2')
        
        # 1. Instanciar el modelo "vacío" (cuerpo + LSTM)
        self.model = BiLBERTLarge()
    ...
    def extract(self, text: str, questions: str) -> Tuple[Dict[str, str], Dict[str, float]]:
        ...
        with torch.no_grad(): # Apagamos gradientes para máxima velocidad
            for key, question in parsed_questions.items():
                ...
                token_type_ids_all = inputs["token_type_ids"].to(self.device)
                ...
                for i in range(num_chunks):
                    ...
                    token_type_ids = token_type_ids_all[i].unsqueeze(0)
